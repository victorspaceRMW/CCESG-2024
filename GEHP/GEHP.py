# -*- coding: utf-8 -*-
"""
Created on Mon Dec  4 15:20:14 2023
...
gas_cost_heating=furnace["heating (gas)"]
gas_cost_heating=gas_cost_heating/1000000

gas_cost_cooling=furnace["cooling (gas)"]
gas_cost_cooling=gas_cost_cooling/1000000

fans=furnace["fans (Electricity)"]
fans=fans/kwh_convert

#print (gas_cost_heating)
#print (gas_cost_cooling)
...
fig.add_trace(go.Bar(
    ...
    name='Electricity Energy Consumption by the fans'))
fig.add_trace(go.Bar(
    x=months,
    y=list(furnace["inverse_heating"]),
    base=0,  # 默认基准设置
    marker_color='lightslategrey',
    name='Gas Energy Consumption, heating'
))
fig.add_trace(go.Bar(
    x=months,
    y=list(furnace["inverse_cooling"]),
    base=0,  # 默认基准设置
    marker_color='blue',
    name='Gas Energy Consumption, cooling'
))
fig.update_layout(
...
bar_gas_cost_heating={"gas_cost_heating":gas_cost_heating,
              "level1":list_l1,
              "level2":list_l2,
              "level3":list_l3,
              "level4":list_l4
    }
bar_gas_cost_heating_df=pd.DataFrame(bar_gas_cost_heating)
#print (bar_gas_cost_heating_df)

gas_cash_list_heating=[]

for index,row in bar_gas_cost_heating_df.iterrows():
    #print (list(row))
    sub_list=list(row)
    ...
    gas_cash_list_heating.append(gas_cash)
print (gas_cash_list_heating)
...
df_cash_GEHP=pd.DataFrame({"Month":month,"elec_cash":overall_cost_electricity,"gas_cash_cooling":gas_cash_list_cooling, "gas_cash_heating":gas_cash_list_heating})
fig = px.bar(df_cash_GEHP,
            x="Month",
            y=["elec_cash","gas_cash_cooling","gas_cash_heating"],title="The Cash cost of GEHP")

df_cash_GEHP.to_csv("Cash, GEHP.csv")

fig.update_layout(
    xaxis_title="Month", yaxis_title="Cash Cost [AUD]",title="Cash cost, GEHP")
